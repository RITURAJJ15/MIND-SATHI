...
export const HomePage: React.FC<HomePageProps> = ({ onNavigate }) => {
  ...
  const handleSetHomeLocation = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        alert(res.error || 'Failed to save home location.');
      }
    } catch (err: any) {
      console.warn('Set home location error:', err);
      alert(
        err?.message ||
          'Unable to get your location. Please ensure location permissions are enabled in your browser settings.'
      );
    } finally {
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="text-left min-w-0 pr-1">
              ...
              <button
                ...
                className="text-[10px] xs:text-xs text-blue-200 bg-[#081F38]/90 hover:bg-[#0E2E50] border border-blue-400/40 rounded-full px-2.5 py-0.5 font-bold mt-1 flex items-center gap-1.5 w-fit transition-all cursor-pointer"
                title={homeLocation ? 'View Home Location on OpenStreetMap' : 'Set Home Location'}
              >
                ...
                <span>
                  {loadingLocation
                    ? 'Home: Checking...'
                    : homeLocation
                    ? `Home: ${homeLocation.locationName || homeLocation.locality || homeLocation.city || 'Saved'}`
                    : 'Home: (Set)'}
                </span>
              </button>
            </div>
      ...
      {/* Dedicated Patient Home Location Dashboard Card */}
      <div className="bg-gradient-to-r from-blue-50 via-sky-50 to-indigo-50 border-2 border-blue-200/90 p-4 sm:p-5 rounded-3xl shadow-xs animate-fade-in">
            ...
            <div>
              ...
              <div className="text-xs text-gray-600 font-medium mt-1">
                {homeLocation ? (
                  ...
                  <span>
                    No home location set yet. Save your safe anchor so loved ones know where your home is.
                  </span>
                )}
              </div>
            </div>
            ...
            {homeLocation ? (
              <>
                <button
                  ...
                  className="px-4 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-extrabold shadow-tactile transition-all cursor-pointer flex items-center gap-1.5 active:translate-y-0.5"
                >
                ...
                <button
                  ...
                  className="px-3.5 py-2.5 rounded-2xl bg-white hover:bg-gray-50 text-gray-700 hover:text-gray-900 border-2 border-gray-300 text-xs font-bold shadow-xs transition-all cursor-pointer disabled:opacity-50"
                  title="Update home coordinates using current browser location"
                >
                  {savingLocation ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : 'Change'}
                </button>
              </>
            ) : (
              <button
                ...
                onClick={handleSetHomeLocation}
                disabled={savingLocation}
                className="px-5 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-extrabold shadow-tactile transition-all cursor-pointer flex items-center gap-2 active:translate-y-0.5 disabled:opacity-50"
              >
                {savingLocation ? (
                  <>
      ...
      {/* Interactive OpenStreetMap Home Location Modal */}
